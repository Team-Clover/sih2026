...
const severity = { 'VERY HIGH': 2, HIGH: 1 }
const sortAlerts = (alerts) => alerts.sort((a, b) => (severity[b.riskLevel] || 0) - (severity[a.riskLevel] || 0) || new Date(b.createdAt) - new Date(a.createdAt))
router.get('/', requireAuth, async (req, res, next) => { try { const alerts = await Alert.find(query(req)).sort({ createdAt: -1 }).limit(50); res.json({ alerts: sortAlerts(alerts) }) } catch (e) { next(e) } })
router.get('/active', requireAuth, async (req, res, next) => { try { const alerts = await Alert.find(query(req)).sort({ createdAt: -1 }).limit(20); res.json({ alerts: sortAlerts(alerts) }) } catch (e) { next(e) } })
